...
try:
    while True:
        ...
        # print(dept.get_aligned_images(dept))
        ...
        # depth_value = depth_image[ff.find_res(depth_colormap)[1]//2,ff.find_res(depth_colormap)[0]//2 ]
            
            # print(f"Depth at pixel ({ff.find_res(depth_colormap)[0]//2}, {ff.find_res(depth_colormap)[1]//2}): {depth_value} millimeters")
            cv2.circle(depth_colormap,(ff.find_res(depth_colormap)[0]//2, ff.find_res(depth_colormap)[1]//2),2,(255,255,255),3)
            # distance = depth_value
            # color_img  = cv2.resize(color_image,(1080,720))
    
        cv2.imshow('RealSense Roll -10 Deg', color_image)
